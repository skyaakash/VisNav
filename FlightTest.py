...
class ImagProc:
	def IP(self,useRPi,Yaw,WpNo,IpCalDrift,ImProFail,awb,ex,photo_width,photo_height,fn,stop_event,ImCapEvent,tiout_event,impro_event):
			...
			if TiOutFlag == 0:
				if ser == 0:		
					while WpCounter > 1:
						...
						X1 = WpC[1+len(WpC)-WpCounter][0]
						Y1 = WpC[1+len(WpC)-WpCounter][1]
						if WpCounter > 2:
							X2 = WpC[2+len(WpC)-WpCounter][0]
							Y2 = WpC[2+len(WpC)-WpCounter][1]
						else:
							X2 = WpC[0][0]
							Y2 = WpC[0][1]
						...
						TimeUpd[i] = (math.sqrt(pow((X2 - X1),2) + pow((Y2 - Y1),2))/Asp)
						RollCd = 0
						RollConst = 0
